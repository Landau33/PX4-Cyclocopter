...
#include "CCAttitudeControl/AttitudeControlMath.hpp"
...
void CyclocopterAttitudeControl::parameters_updated()
{
	...
	// 角速度限制
	using math::radians;
	_attitude_control.setRateLimit(Vector3f(radians(_param_cc_rollrate_max.get()), radians(_param_cc_pitchrate_max.get()),
						radians(_param_cc_yawrate_max.get())));
...
void CyclocopterAttitudeControl::generate_attitude_setpoint(const Quatf &q, float dt, bool reset_yaw_sp)
{
	...
	attitude_setpoint.yaw_sp_move_rate = _manual_control_setpoint.yaw * math::radians(_param_cc_yawrate_max.get());
