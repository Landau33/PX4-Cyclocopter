...
#include "CCPositionControl/ControlMath.hpp"
...
void CyclocopterPositionControl::parameters_update(bool force)
{
	...
	if (_parameter_update_sub.updated() || force) {
		...
		if (_param_sys_vehicle_resp.get() >= 0.f) {
			...
			num_changed += _param_cpc_yawrate_max.commit_no_notification(math::lerp(80.f, 450.f, responsiveness));
			num_changed += _param_cpc_z_acc_max_up.commit_no_notification(math::lerp(1.f, 15.f, responsiveness));
			num_changed += _param_cpc_z_acc_max_dn.commit_no_notification(math::lerp(0.8f, 15.f, responsiveness));
		...
		if (_param_cpc_z_vel_all.get() >= 0.f) {
			...
			num_changed += _param_cpc_z_vel_max_dn.commit_no_notification(z_vel * 0.75f);
		...
		_param_cpc_land_speed.set(math::min(_param_cpc_land_speed.get(), _param_cpc_z_vel_max_dn.get()));
		...
		_goto_control.setParamCpcZAccMaxDown(_param_cpc_z_acc_max_dn.get());
		_goto_control.setParamCpcXyErrMax(_param_cpc_xy_err_max.get());
		_goto_control.setParamCpcXyVelMax(_param_cpc_xy_vel_max.get());
		_goto_control.setParamCpcZVelUp(_param_cpc_z_vel_max_up.get());
		_goto_control.setParamCpcZVelDn(_param_cpc_z_vel_max_dn.get());
		// 配置起飞参数
...
void CyclocopterPositionControl::Run()
{
	...
	if (_local_pos_sub.update(&vehicle_local_position)) {
		...
		if (_vehicle_control_mode.flag_multicopter_position_control_enabled
			...
			if (!PX4_ISFINITE(_vehicle_constraints.speed_up) || (_vehicle_constraints.speed_up > _param_cpc_z_vel_max_up.get())) {
				_vehicle_constraints.speed_up = _param_cpc_z_vel_max_up.get();
			}
			// 处理平滑起飞
			bool skip_takeoff = false;
			...
			if (!_control.update(dt)) {
				...
				_control.setVelocityLimits(_param_cpc_xy_vel_max.get(), _param_cpc_z_vel_max_up.get(), _param_cpc_z_vel_max_dn.get());
				_control.update(dt);
			}
