...
void GotoControl::setPositionSmootherLimits(const goto_setpoint_s &goto_setpoint)
{
	...
	float max_horizontal_accel = _param_cpc_xy_acc; // 最大水平加速度
	...
	if (goto_setpoint.flag_set_max_horizontal_speed
		...
		if (!_position_smoothing.getCurrentVelocityXY().longerThan(max_horizontal_speed)) {
			...
			max_horizontal_accel = math::constrain(_param_cpc_xy_acc * speed_scale, 0.f, _param_cpc_xy_acc);
		}
	}
	...
	// 垂直约束
	float vehicle_max_vertical_speed = _param_cpc_z_vel_max_dn; // 最大垂直下降速度
	float vehicle_max_vertical_accel = _param_cpc_z_acc_max_dn; // 最大垂直下降加速度
...
{
	float max_heading_rate = _param_cpc_yawrate_max; // 最大航向速率
	...
	if (goto_setpoint.flag_set_max_heading_rate && PX4_ISFINITE(goto_setpoint.max_heading_rate)) {
		max_heading_rate = math::constrain(goto_setpoint.max_heading_rate, 0.f, _param_cpc_yawrate_max); // 限制最大航向速率

		// 根据航向速率限制线性缩放航向加速度限制以保持平滑动态
		if (fabsf(_heading_smoothing.getSmoothedHeadingRate()) <= max_heading_rate) {
			const float rate_scale = max_heading_rate / _param_cpc_yawrate_max;
